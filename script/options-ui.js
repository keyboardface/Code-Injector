...
/** 
 * check if the given string contains code (tripping the comments)
 * 
 * @param {string} _string 
 */
...
function getRules(_cb){
    chrome.storage.local.get('rules').then(function(_data){
...
function setRules(_rules, _ok, _ko){
    chrome.storage.local.set({ rules: _rules }).then(_ok).catch(_ko);
}
...
function updateSettings(){
    return chrome.storage.local.get('settings').then(function(_data){
...
/**
 * trigger an opacity effect for the info message
 */
...
/**
 * get the remote rule from github 
 * 
...
window.addEventListener('load', function(_e){
        ...
        switch(target.dataset.name){
            ...
            case 'btn-clear-rules': 
                if (target.dataset.confirm === 'true'){
                    ...
                    chrome.storage.local.set({ rules: [] }).then(function(){
                        updateRulesCounter([]);
                    });
                }
                else{
            ...
            case 'btn-import': 
                ...
                p.then(
                    function(_res){ // OK
                        ...
                        // set the number of imported rules
    ...
    chrome.storage.onChanged.addListener(function(_changes, _namespace) {
        if (_namespace === 'local' && _changes.rules && _changes.rules.newValue) {
            updateRulesCounter(_changes.rules.newValue);
        }
